...
def imgGeneration(folder_path, lon, lat, lon_list, lat_list, truth_time, h8_ch2, h8_ch3, h8_ch4,
                  h8_ch7, h8_ch7_minus_14, img_save=True, img_csv=False):
    truth_lon = lon
    truth_lat = lat
    ...
    if img_save:
        misc.imsave(folder_path + '/band2' + '_' + str(truth_lon) + '_' + str(truth_lat) + '.jpg',
                    h8_ch2[lat_start:lat_end, lon_start:lon_end])
        pd.DataFrame(h8_ch2[lat_start:lat_end, lon_start:lon_end]).to_csv(
            folder_path + '/band2' + '_' + str(truth_lon) + '_' + str(truth_lat) + '.csv')

        misc.imsave(folder_path + '/band3' + '_' + str(truth_lon) + '_' + str(truth_lat) + '.jpg',
                    h8_ch3[lat_start:lat_end, lon_start:lon_end])
        pd.DataFrame(h8_ch3[lat_start:lat_end, lon_start:lon_end]).to_csv(
            folder_path + '/band3' + '_' + str(truth_lon) + '_' + str(truth_lat) + '.csv')

        misc.imsave(folder_path + '/band4' + '_' + str(truth_lon) + '_' + str(truth_lat) + '.jpg',
                    h8_ch4[lat_start:lat_end, lon_start:lon_end])
        pd.DataFrame(h8_ch4[lat_start:lat_end, lon_start:lon_end]).to_csv(
            folder_path + '/band4' + '_' + str(truth_lon) + '_' + str(truth_lat) + '.csv')

        misc.imsave(folder_path + '/band7' + '_' + str(truth_lon) + '_' + str(truth_lat) + '.jpg',
                    h8_ch7[lat_start:lat_end, lon_start:lon_end])
        pd.DataFrame(h8_ch7[lat_start:lat_end, lon_start:lon_end]).to_csv(
        ...
        misc.imsave(folder_path + '/band7_14' + '_' + str(truth_lon) + '_' + str(truth_lat) + '.jpg',
                    h8_ch7_minus_14[lat_start:lat_end, lon_start:lon_end])
        pd.DataFrame(h8_ch7_minus_14[lat_start:lat_end, lon_start:lon_end]).to_csv(
...
def POI_to_params(POI_time, argsDict):
    ...
    h8_ch14 = h8_data[15]
    h8_ch15 = h8_data[16]
    h8_ch7_minus_14 = h8_ch7 - h8_ch14
    h8_ch3_plus_4 = h8_ch3 + h8_ch4
    lon_list = [round(item, 2) for item in list(lon)]
    lat_list = [round(item, 2) for item in list(lat)]

    # 云掩膜
    f = interpolate.interp1d(np.array([0, 0.12, 0.24, 0.47, 0.75, 1]), np.array([0, 0.43, 0.63, 0.82, 0.94, 1]),
                             kind="cubic")  # 三次样条插值
    ch1_refl = f(h8_ch1)
    ch3_refl = f(h8_ch3)
    ch4_refl = f(h8_ch4)
    vci = 255 * sqrt(1 / 3) * np.sqrt(
        np.power(ch1_refl - ch3_refl, 2) + np.power(ch1_refl - ch4_refl, 2) + np.power(ch3_refl - ch4_refl, 2))
    # 对于陆地，云判的阈值为22
    cloud_mask = vci < 22
    # 误差修正
    # 若像元7通道亮温值高于13通道亮温值12K(陆地)，视当前像元受卷云影响
    # 若像元4通道反射率小于0.18(陆地)，当前像元被视为戈壁沙漠，云掩膜结果将剔除该像元
    cloud_mask = np.logical_or(cloud_mask, h8_ch7 - h8_ch13 > 12)
    cloud_mask = np.logical_and(h8_ch4 >= 0.18, cloud_mask)
    # 读取上一帧的pk影像，用于绝对火点判定
    if os.path.exists(pk_path_last):
        with open(pk_path_last, 'rb') as h8_f_last:
            h8_data_last = pickle.load(h8_f_last)
            h8_ch7_last = h8_data_last[8]
            h8_ch14_last = h8_data_last[15]
            h8_ch7_minus_14_last = h8_ch7_last - h8_ch14_last
            band7_deta2 = h8_ch7 - h8_ch7_last
            band7_14_deta2 = h8_ch7_minus_14 - h8_ch7_minus_14_last
    ...
    for fire_lon_index in list(range(len(lon_list)))[yunnan_min_lon_index:yunnan_max_lon_index + 1]:
        for fire_lat_index in list(range(len(lat_list)))[yunnan_max_lat_index:yunnan_min_lat_index + 1]:
            ...
            if os.path.exists(pk_path_last):
                ...
                band7_14_deta = -999
            # 若该像元为云像元，跳过
            if cloud_mask[fire_lat_index, fire_lon_index] == 1:
                continue
            ...
            kernel_cloud = cloud_mask[x_start:x_start + size, y_start:y_start + size]
            ...
            x_start = fire_lat_index - kernal_point
            y_start = fire_lon_index - kernal_point
            kernel_cloud_edge = cloud_mask[x_start:x_start + 5, y_start:y_start + 5]
            if np.sum(kernel_cloud_edge) > 6:
                continue
            # 排除7通道亮温值过低的火点
            if h8_ch7[fire_lat_index, fire_lon_index] <= 260:
                continue
            # 计算像元的若干特征，用于火点判别
            ...
            band7_mean_size7, band7_14_mean_size7, n_cloud, n_cloud_band2 = fdj.hd_test(h8_ch3,
                                                                                        h8_ch7,
                                                                                        h8_ch14,
                                                                                        fire_lat_index,
                                                                                        fire_lon_index,
                                                                                        ...
                                                                                        h8_ch2, kernel_cloud,
                                                                                        size=15)
            band3_data = h8_ch3[fire_lat_index, fire_lon_index]  # 获取3通道反射率
            band4_data = h8_ch4[fire_lat_index, fire_lon_index]  # 获取4通道反射率
    ...
    if cluster_flag and output_temp.shape[0] > 1:
        ...
        for i in range(output_dbscan.shape[0]):
            ...
            imgGeneration(folder_path, lon_db_temp, lat_db_temp, lon_list, lat_list, time_str, h8_ch2,
                          h8_ch3, h8_ch4, h8_ch7, h8_ch7_minus_14, img_save=True, img_csv=False)
    ...
    fire_absolute_df.to_csv("./fire_detection/output_absolute/" + csv_name_absolute)
